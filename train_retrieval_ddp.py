...
database_sampler = torch.utils.data.distributed.DistributedSampler(database_dataset,shuffle=False)
query_sampler = torch.utils.data.distributed.DistributedSampler(query_dataset,shuffle=False)
trainloader = DataLoader(train_dataset ,batch_size = CFG.train_bs,num_workers = 8,pin_memory=True,sampler=train_sampler)
databaseloader = DataLoader(database_dataset ,batch_size = CFG.train_bs,num_workers = 8,pin_memory=True,sampler=database_sampler)
queryloader = DataLoader(query_dataset ,batch_size = CFG.train_bs,num_workers = 8,pin_memory=True,sampler=query_sampler)

#create model
# model = CreateClipVitModel(in_features=768,df=train_df)
model = CreateClipOpenAiModel(in_features=1024)
model.load_state_dict(torch.load('/root/autodl-tmp/trained_models/clip_vitH_14_openai_newData_state_dict.pt'))
...
model = torch.nn.parallel.DistributedDataParallel(model.float().cuda(), device_ids=[args.local_rank])
...
# train
max_mP_at_5 = -inf
logger = get_logger('/root/GUIE/GUIE/logs/clip_vitH_14_ddp_openai_newData_embeddim256.log')
for i in range(CFG.epochs):
    # train
    model.train()
    scaler = amp.GradScaler()
    for idx,(images,labels) in enumerate(trainloader):
        images = images.to(torch.float32).cuda(non_blocking=True)
        labels = labels.to(torch.long).cuda(non_blocking=True)

        with amp.autocast(enabled=True):
            pred = model.module(images,labels)
    ...
    with torch.no_grad():
        ...
        for idx,inp in enumerate(final_iter):
            ...
            database_target_labels.extend(target.cpu().numpy().tolist())
            out = F.normalize(model.module.feature_extractor(input_img.to(CFG.device)))
        ...
        for idx,inp in enumerate(final_iter):
            input_img,target = inp[0].to(torch.float), inp[1].to(torch.long)
            q_target_labels.extend(target.cpu().numpy().tolist())
            out = F.normalize(model.module.feature_extractor(input_img.to(CFG.device)))
    ...
    torch.save(model.module.state_dict(),'/root/autodl-tmp/trained_models/clip_vitH_14_openai_ddp_newData_state_dict.pt')